...
```mermaid
flowchart TD
    Start["URL アクセス"] --> Host["ホスト名 → サイトキー<br/>resolveSiteKey()"]

    Host -->|ブロック設定のあるサイトに属さない| Unblocked["✅ 許可"]
    Host -->|属する| A{"グローバル一時停止？<br/>settings.paused"}

    A -->|Yes| Unblocked
    A -->|No| D{"スケジュール内？<br/>isAnyScheduleActive()"}

    D -->|No（スケジュールがあり、どれも時間外）| Unblocked
    D -->|Yes（スケジュール 0 件を含む）| C{"ブロック有効？<br/>rule.enabled"}
...
`src/lib/blockService.ts` は保存値（`settings` と `activity`）を読み、サイトキーごとの
ブロック設定と今日の表示秒数を `evaluateBlock()` に渡すだけにする。
...
- ルール生成はブロック設定を持つすべてのサイトを `evaluateBlock()` に通し、ブロックするものの
  サイトキーを返す。ルールは `||サイトキー` で、本体とすべてのサブドメインを止める
- スケジュールが 1 件以上あれば、すべての項目はスケジュール内だけブロックする
  （時間制限の無い項目もスケジュール外は閲覧できる）。スケジュール外でも滞在時間は記録するが、
  制限には使わない

### サイトキー

ブロックリストの項目は、`*.` と `www.` を除いて小文字にしたドメイン（サイトキー）に揃えてから照合する
（`src/lib/siteKey.ts`）。ホスト名は「キーと一致するか `.キー` で終わる」最も長いキーに属する。

- `*.example.com` / `www.example.com` / `example.com` はどれも `example.com` になる
- 同じサイトキーの項目が複数あれば、ブロックリストで先にある項目の設定を使う
- 時間制限の使用量もサイトキーで数える。`www.example.com` と `m.example.com` の滞在は同じ枠に入る
...
- ブロックリストに `youtube.com` と同じサイトキーの項目があれば、そちらの設定が優先される
