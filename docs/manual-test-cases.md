...
- [ ] スケジュール重複エラー
  ...
  - 期待: 「既存のスケジュールと重複しています。」エラーメッセージが表示され、保存されない
...
- [ ] プリセット削除時のスケジュール連携解除
  - 手順: プリセットを削除 → 確認モーダルで件数を確認して削除を確定 → そのプリセットを使用していたスケジュールを確認
  - 期待: 確認モーダルに参照件数が表示される。削除後もスケジュールは残り、有効・無効の状態も変わらず、スタイル表示だけが消える
