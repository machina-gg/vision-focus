...
interface SiteCategoryManagerProps {
  ...
  onCategoryChange: (domain: string, category: 'waste' | 'invest' | 'neutral') => void
}

type SortKey = 'time' | 'domain' | 'category'
type SortOrder = 'asc' | 'desc'

const CATEGORY_OPTIONS: { value: 'waste' | 'invest' | 'neutral'; label: string; color: string }[] = [
  { value: 'waste', label: 'Waste', color: 'text-red-600 bg-red-50 border-red-200' },
  { value: 'invest', label: 'Invest', color: 'text-green-600 bg-green-50 border-green-200' },
  { value: 'neutral', label: 'Neutral', color: 'text-gray-600 bg-gray-50 border-gray-200' },
]
...
export function SiteCategoryManager({
  ...
  const [filterCategory, setFilterCategory] = useState<'all' | 'waste' | 'invest' | 'neutral'>('all')

  // Get all sites with their time and category
  const sites = useMemo(() => {
    const siteList: (SiteTime & { category: 'waste' | 'invest' | 'neutral' })[] = []

    for (const [domain, siteTime] of Object.entries(analytics.siteTime)) {
      const category = analytics.siteCategories[domain] || siteTime.category || 'neutral'
      siteList.push({
        ...siteTime,
        domain,
        category,
  ...
  const filteredSites = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter((site) => site.domain.toLowerCase().includes(query))
    }
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="p-3 bg-red-50 rounded-lg border border-red-100">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-medium text-red-600">{getMessage('waste')}</span>
          </div>
          <p className="text-lg font-bold text-red-700">{formatTime(totals.waste)}</p>
        </div>
        <div className="p-3 bg-green-50 rounded-lg border border-green-100">
          <div className="flex items-center gap-2 mb-1">
            <TrendingUp className="w-4 h-4 text-green-500" />
            <span className="text-xs font-medium text-green-600">{getMessage('invest')}</span>
          </div>
          <p className="text-lg font-bold text-green-700">{formatTime(totals.invest)}</p>
        </div>
        <div className="p-3 bg-gray-50 rounded-lg border border-gray-100">
          <div className="flex items-center gap-2 mb-1">
            <Minus className="w-4 h-4 text-gray-400" />
            <span className="text-xs font-medium text-gray-600">{getMessage('neutral')}</span>
          </div>
          <p className="text-lg font-bold text-gray-700">{formatTime(totals.neutral)}</p>
        </div>
      </div>
            ...
            filteredSites.map((site) => (
              ...
              >
                <div className="col-span-5 flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-sm text-gray-900 truncate" title={site.domain}>
                    {site.domain}
                  </span>
                </div>
                ...
                <div className="col-span-4">
                  <div className="flex gap-1">
                    {CATEGORY_OPTIONS.map((option) => (
                      <button
                        key={option.value}
                        onClick={() => onCategoryChange(site.domain, option.value)}
                      ...
                      >
                        ...
                        {option.value === 'neutral' && getMessage('neutralShort')}
                      </button>
                    ))}
                  </div>
                </div>
