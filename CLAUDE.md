...
- 保存データ・エクスポート JSON の後方互換コード（項目が無ければ既定値で補う・旧形式の読み替え・任意項目にして受ける）。リリース前は新項目を必須にし、古いデータは捨てる
...
- 早期リターンでネストを減らす
- マジックナンバーは定数化
- コメントは原則書かない。書くのは、コードとテストのどちらからも読み取れず、崩すと気づかれずに壊れる制約だけ（1〜2 行）。経緯・Issue 番号は書かない
- JSDoc はコメントとは別扱いで、export するもの（関数・コンポーネント・フック・型・定数）すべてに書く。名前で分かるものも省かない。関数は要約に加えて @param / @returns、型はフィールドごとに 1 行を書く
