...
| 要素           | 実装                                                                    |
| -------------- | ----------------------------------------------------------------------- |
| URL・猶予期間  | `src/constants/support.ts`                                              |
| 表示状態の管理 | `src/lib/supportPrompt.ts`（`chrome.storage.local` の `supportPrompt`） |
| 表示制御フック | `src/hooks/useSupportPrompt.ts`                                         |
| UI             | `src/components/features/Support/`                                      |

- ボタンは Buy Me a Coffee の公式ウィジェット JS も外部ホストの画像も使わず、ブランドカラーとアイコンで自前に構成する。外部画像の読み込みもネットワークアクセスになるため
- 保存するのは「閉じた時刻」と「支援ページを開いたか」のみ。外部送信はしない
- レポート下の誘導は 30 日に 1 回まで。支援ページを開いた場合は以降表示しない
