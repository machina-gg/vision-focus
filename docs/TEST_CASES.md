# E2Eテストケース

## 1. テスト概要

| 項目         | 内容                                                               |
| ------------ | ------------------------------------------------------------------ |
| 対象ブラウザ | Chrome（Desktop）                                                  |
| テストツール | Playwright                                                         |
| 実行コマンド | `pnpm test:e2e`（ヘッドレス） / `pnpm test:e2e:headed`（表示付き） |
| テスト対象   | Chrome拡張機能（Manifest V3）                                      |

### テスト環境の注意点

Chrome拡張機能のE2Eテストには以下の特殊な設定が必要：

- Playwright の `chromium` を使用し、拡張機能をロード
- `--disable-extensions-except` と `--load-extension` フラグを使用
- Service Worker のテストには `chrome.runtime` API のモックが必要な場合あり

### テストケースの件数

件数は下の一覧そのものが SSOT。ここに数字を書くと一覧の増減と必ずずれるため、
必要なときは次のコマンドで数える。

```bash
# 優先度ごとの件数（ID 列を持つ行だけを数える）
grep -oE '^\| [A-Z0-9-]+ +\|.*\| (P[012]) +\|' docs/TEST_CASES.md |
  grep -oE 'P[012]' | sort | uniq -c
```

## 2. テストケース一覧

### ポップアップ画面（popup）

| ID      | シナリオ                                                     | 優先度 |
| ------- | ------------------------------------------------------------ | ------ |
| POP-001 | ポップアップが正常に表示される                               | P0     |
| POP-002 | ヘッダーにロゴと設定アイコンが表示される                     | P1     |
| POP-003 | 目標カードに目標テキストが表示される                         | P0     |
| POP-004 | 今日のサマリー（ブロック回数、トップブロックサイト）が表示   | P0     |
| POP-005 | 設定アイコンクリックでオプション画面が開く                   | P1     |
| POP-006 | 目標カードクリックでダッシュボード（新規タブ）が開く         | P1     |
| POP-007 | ヘルプアイコンクリックでヘルプタブが開く                     | P2     |
| POP-008 | Pause トグルでブロック機能の一時停止ができる                 | P1     |
| POP-009 | パスワード保護設定時、Pause トグルにパスワード認証が必要     | P1     |
| POP-010 | クイックブロックボタンに現在のドメインが表示される           | P1     |
| POP-011 | クイックブロッククリックでサイトがブロックリストに追加される | P1     |
| POP-013 | Analytics リンクが表示される                                 | P2     |
| POP-014 | Time Limit 設定中のサイトで残り時間バッジが表示される        | P1     |

### ダッシュボード / 新規タブ（newtab）

| ID      | シナリオ                                                 | 優先度 |
| ------- | -------------------------------------------------------- | ------ |
| NEW-001 | 新規タブを開くとダッシュボードが表示される               | P0     |
| NEW-002 | プリセット設定済み時、背景画像が表示される               | P0     |
| NEW-003 | 目標テキストが中央に表示される                           | P0     |
| NEW-004 | ミニ統計カード（ブロック回数）が表示される               | P1     |
| NEW-005 | 設定アイコンクリックでオプション画面が開く               | P1     |
| NEW-006 | 目標テキストをダブルクリックで編集モードになる           | P1     |
| NEW-007 | 編集した目標がEnterキーで保存される                      | P1     |
| NEW-008 | プリセット未設定時も通常のダッシュボードとCTAが表示      | P0     |
| NEW-009 | ブロックされたサイトから遷移時、ブロック情報が表示される | P1     |
| NEW-010 | ブロックサイトリストが表示される                         | P1     |
| NEW-011 | 壁紙ダウンロードボタンが表示される                       | P2     |
| NEW-012 | Time Limit 超過からの遷移時、専用メッセージが表示される  | P1     |
| NEW-013 | ブロック日数（Blocking Days）が正しく表示される          | P2     |

### オプション画面（options）

#### タブ共通

| ID      | シナリオ                                          | 優先度 |
| ------- | ------------------------------------------------- | ------ |
| OPT-001 | オプション画面が正常に開く                        | P0     |
| OPT-002 | タブ切り替えが正常に動作する                      | P0     |
| OPT-003 | URL ハッシュでタブ指定ができる（例: #analytics）  | P1     |
| OPT-004 | Analytics Opt-In モーダルが初回訪問時に表示される | P1     |

#### ブロックリストタブ

| ID      | シナリオ                                                                       | 優先度 |
| ------- | ------------------------------------------------------------------------------ | ------ |
| OPT-B01 | ブロックリストタブが表示される                                                 | P0     |
| OPT-B02 | ドメインを入力してブロックリストに追加できる                                   | P0     |
| OPT-B03 | ワイルドカード（\*.example.com）が入力できる                                   | P1     |
| OPT-B04 | ブロックリストの項目を削除できる                                               | P0     |
| OPT-B05 | ブロックリストの項目を有効/無効切り替えできる                                  | P1     |
| OPT-B06 | Time Limit（時間制限）を設定できる                                             | P1     |
| OPT-B07 | パスワード保護設定時、削除・無効化時にパスワード認証が必要                     | P1     |
| OPT-B08 | パスワード未設定時、Unblock 確認モーダルが表示される                           | P1     |
| OPT-B09 | ブロック回数が各ドメインに表示される                                           | P1     |
| OPT-B10 | YouTube セクションが表示される（youtube.com はブロック中のサイト一覧に出ない） | P1     |
| OPT-B11 | YouTube ブロック設定（Shorts/Recommendations/Comments）を切り替え              | P1     |
| OPT-B12 | YouTube Time Limit を設定できる                                                | P1     |

#### スタイル設定タブ

| ID       | シナリオ                                                   | 優先度 |
| -------- | ---------------------------------------------------------- | ------ |
| OPT-ST01 | スタイルタブが表示される                                   | P0     |
| OPT-ST02 | プリセット一覧が表示される                                 | P0     |
| OPT-ST03 | プリセットを選択・適用できる                               | P0     |
| OPT-ST04 | 新規プリセットを作成できる                                 | P1     |
| OPT-ST05 | プリセットを削除できる                                     | P1     |
| OPT-ST06 | 目標テキスト・サブテキストを入力できる                     | P0     |
| OPT-ST07 | テキスト色を選択できる                                     | P1     |
| OPT-ST08 | 背景タイプ（画像/単色）を選択できる                        | P1     |
| OPT-ST09 | デフォルト背景画像を選択できる                             | P1     |
| OPT-ST10 | カスタム画像をアップロードできる                           | P2     |
| OPT-ST11 | フォント設定（ファミリー・サイズ・ウェイト）を変更可能     | P1     |
| OPT-ST12 | Google Fonts を選択できる                                  | P2     |
| OPT-ST13 | 背景画像変更時にリアルタイムプレビューされる               | P2     |
| OPT-ST14 | フォント変更時にリアルタイムプレビューされる               | P2     |
| OPT-ST15 | 色変更時にリアルタイムプレビューされる                     | P2     |
| OPT-ST16 | Free ダウングレード時、4件目以降のプリセットがロックされる | P2     |

#### スケジュールタブ

| ID      | シナリオ                                                       | 優先度 |
| ------- | -------------------------------------------------------------- | ------ |
| OPT-S01 | スケジュールタブが表示される                                   | P0     |
| OPT-S02 | 週間カレンダーが表示される                                     | P1     |
| OPT-S03 | スケジュールを追加できる                                       | P1     |
| OPT-S04 | 時間帯（開始・終了）を設定できる                               | P1     |
| OPT-S05 | 曜日を選択できる                                               | P1     |
| OPT-S06 | プリセットをスケジュールに連携できる                           | P1     |
| OPT-S07 | スケジュールを編集できる                                       | P1     |
| OPT-S08 | スケジュールを削除できる                                       | P1     |
| OPT-S09 | スケジュールを有効/無効切り替えできる                          | P1     |
| OPT-S10 | 設定した時間帯に該当プリセットが自動適用される                 | P0     |
| OPT-S11 | 無効なスケジュールは半透明・取り消し線で表示される             | P1     |
| OPT-S12 | 全てのスタイルがスケジュールで選択できる                       | P2     |
| OPT-S13 | 重複スケジュール（同時刻・同曜日）が設定された場合にエラー表示 | P1     |
| OPT-S14 | プリセット削除時、該当スケジュールのスタイル連携が解除される   | P1     |

#### 分析タブ

| ID      | シナリオ                                                                                                 | 優先度 |
| ------- | -------------------------------------------------------------------------------------------------------- | ------ |
| OPT-A01 | 分析タブが表示される                                                                                     | P0     |
| OPT-A02 | サイト別ランキングがブロック回数の多い順に表示される（日をまたいで足し上げる）                           | P1     |
| OPT-A03 | 追跡中のサイト一覧が表示される（ブロック中 / 無効 / 解除済みの状態と、無効の行の再ブロック・停止ボタン） | P1     |
| OPT-A04 | 解除サイトの解除後の時間（最後に解除した日から今日までの表示時間）が表示される                           | P1     |
| OPT-A05 | 解除サイトを再ブロックできる                                                                             | P1     |
| OPT-A06 | トラッキング停止ができる（無効にしたサイトはブロックリストから外してから止める）                         | P2     |
| OPT-A07 | 新規サイトをトラッキングに追加できる（入れ子は拒否し、理由を表示して入力を残す）                         | P2     |
| OPT-A08 | Analytics データをリフレッシュできる                                                                     | P2     |
| OPT-A09 | Analytics データをリセットできる                                                                         | P2     |
| OPT-A10 | CSV エクスポートができる（ブロックリスト・統計データ）                                                   | P2     |
| OPT-A11 | 解除サイトの CSV エクスポートが可能（解除の記録がある）                                                  | P2     |
| OPT-A12 | 週次・月次レポートをタブで切り替え、見ていた期間を保つ                                                   | P2     |

#### 設定タブ

| ID        | シナリオ                                                               | 優先度 |
| --------- | ---------------------------------------------------------------------- | ------ |
| OPT-SET01 | 設定タブが表示される                                                   | P1     |
| OPT-SET02 | パスワード設定セクションが表示される                                   | P1     |
| OPT-SET03 | パスワードを設定できる                                                 | P1     |
| OPT-SET04 | パスワードを変更できる                                                 | P1     |
| OPT-SET05 | パスワードを削除できる                                                 | P1     |
| OPT-SET06 | 長押しの秒数を変えると、ブロック解除にその秒数の長押しが要る           | P2     |
| OPT-SET07 | 通知設定を変更できる（時間制限つきのサイトが無くても表示される）       | P2     |
| OPT-SET08 | Analytics Opt-In 設定を変更できる                                      | P2     |
| OPT-SET09 | 設定データをエクスポートできる                                         | P2     |
| OPT-SET10 | 設定データをインポートできる                                           | P2     |
| OPT-SET11 | インポートでブロック対象になったサイトが、開いていたタブでも置き換わる | P2     |

#### ヘルプタブ

| ID      | シナリオ                   | 優先度 |
| ------- | -------------------------- | ------ |
| OPT-H01 | ヘルプタブが表示される     | P1     |
| OPT-H02 | 基本的な使い方が表示される | P2     |
| OPT-H03 | FAQ が表示される           | P2     |

### サイトブロック機能

| ID        | シナリオ                                                    | 優先度 |
| --------- | ----------------------------------------------------------- | ------ |
| BLOCK-001 | ブロックリストに追加したサイトが newtab.html にリダイレクト | P0     |
| BLOCK-002 | ワイルドカードで指定したサブドメインがブロックされる        | P1     |
| BLOCK-003 | ブロックリストから削除したサイトにアクセスできる            | P0     |
| BLOCK-004 | Pause トグルで全ブロックが一時停止される                    | P0     |
| BLOCK-005 | Pause 解除後、通常のブロック動作に戻る                      | P1     |
| BLOCK-006 | 無効化したブロックアイテムはブロックされない                | P1     |
| BLOCK-007 | 有効化したブロックアイテムがブロックされる                  | P1     |
| BLOCK-008 | declarativeNetRequest でリダイレクトが実行される            | P0     |
| BLOCK-009 | ブロック時にリダイレクト先でブロック元ドメインが表示される  | P2     |
| BLOCK-010 | ブロック回数がカウントされる                                | P1     |

### Time Limit 機能

| ID     | シナリオ                                             | 優先度 |
| ------ | ---------------------------------------------------- | ------ |
| TL-003 | Time Limit 超過時に newtab.html へリダイレクトされる | P0     |
| TL-006 | 残り時間がポップアップで表示される                   | P1     |
| TL-007 | Time Limit の残り時間がブロックリストに表示される    | P2     |
| TL-008 | Pause 有効中は Time Limit 超過してもブロックされない | P1     |
| TL-009 | Daily の使用実績は日付が変わるとリセットされる       | P1     |
| TL-011 | 複数サイトで異なる Time Limit が同時に動作する       | P1     |

### YouTube ブロック機能

| ID     | シナリオ                                             | 優先度 |
| ------ | ---------------------------------------------------- | ------ |
| YT-001 | YouTube Shorts を非表示にできる                      | P1     |
| YT-002 | YouTube Recommendations（関連動画）を非表示にできる  | P1     |
| YT-003 | YouTube Comments を非表示にできる                    | P2     |
| YT-004 | YouTube 完全ブロック（blockAccess）が動作する        | P1     |
| YT-006 | アクセスブロック無効時は Time Limit 超過でも隠さない | P1     |
| YT-007 | YouTube 設定変更が即座に反映される                   | P2     |
| YT-009 | Hide Shorts + Time Limit 同時設定時に両方が機能する  | P1     |
| YT-010 | blockAccess と Time Limit の併用（超過後にブロック） | P1     |
| YT-011 | 有効化を OFF にすると確認が出て、長押しで解除される  | P1     |

### アナリティクス機能

| ID     | シナリオ                                                         | 優先度 |
| ------ | ---------------------------------------------------------------- | ------ |
| AN-001 | サイト別ブロック回数が記録される                                 | P1     |
| AN-002 | ブロックリストから外したサイトは追跡中に残る                     | P1     |
| AN-003 | 解除サイトの滞在時間が Heartbeat で記録される                    | P1     |
| AN-004 | 追跡中サイトのサブドメインで見ている時間はサイトの行に記録される | P2     |
| AN-005 | Analytics Opt-In モーダルで許可/拒否を選択できる                 | P1     |
| AN-006 | Opt-Out でもブロック回数の集計は続く                             | P2     |
| AN-007 | Analytics データをリセットできる                                 | P2     |
| AN-010 | Opt-Out でも解除したサイトの追跡と解除の記録は残る               | P1     |

### 多言語対応

| ID       | シナリオ                                         | 優先度 |
| -------- | ------------------------------------------------ | ------ |
| I18N-001 | ブラウザ言語が英語の場合、英語UIが表示される     | P1     |
| I18N-002 | ブラウザ言語が日本語の場合、日本語UIが表示される | P1     |
| I18N-003 | ヘッダーの操作は実装どおりのものだけ             | P1     |
| I18N-004 | 表示言語が全画面で統一されている                 | P1     |

### 追加機能

かつて有料版限定だった機能。全ユーザーが利用できる。

| ID     | シナリオ                                 | 優先度 |
| ------ | ---------------------------------------- | ------ |
| PR-002 | Google Fonts を選択できる                | P2     |
| PR-003 | カスタム背景画像アップロードが使える     | P2     |
| PR-004 | 壁紙ダウンロード機能が使える             | P2     |
| PR-005 | スタイルを上限（10件）まで作成できる     | P2     |
| PR-006 | Analytics 全期間表示が使える             | P2     |
| PR-007 | Unblock History CSV エクスポートが使える | P2     |

### E2E の外部サイトについて

ブロック機能のテストは実際のサイトへ遷移して確認する必要があるが、外部への実アクセスは行わない。Chromium の `--host-resolver-rules` で全ホストをローカルの HTTPS サーバ（`tests/e2e/fixtures/testServer.ts`）へ向け、`example.com` や `youtube.com` をローカルで再現する。

- 拡張機能側は `declarativeNetRequest` の `||domain` でスキーム非依存に判定するため、実サイトと同じ経路を通る
- HTTPS で待ち受けるのは、`youtube.com` が HSTS プリロード済みで http:// が内部昇格されるため。証明書は実行時に自己署名で生成する（リポジトリには含めない）
- storage の書き込みは Service Worker 経由（`tests/e2e/helpers/sw.ts`）で行う。options を開いて書くとアプリの hydration が state を書き戻して上書きしたり、UI が時間制限値をプリセットに丸めたりするため、意図した状態にならない
- 時間制限の超過判定は `activity` の今日の行（サイトキーごとの `seconds`）を見るが、activity の変更はブロックルール再計算のトリガーにならない。実装と同じ経路（`check-schedule` アラーム）を即時発火させて待つ。使用量の種も `makeActivity()` で置く（日付はローカル日付。日付が変わった状態は前日の行として置く）
- ポップアップの今日のサマリー（POP-004）・新しいタブのミニ統計（NEW-004）・ブロック情報とブロック中のサイト一覧の回数（NEW-009 / NEW-010）は `activity` から導出される。前 2 つは今日の行だけ、後 2 つは保持期間全体を数える。種は `makeActivity()` で置き、**母集団は追跡中のサイト**なので、対象のサイトを同じテストで `makeSites()` にも入れる（入れないと行が読み飛ばされて 0 になる）
- 分析タブ（OPT-A02 / A04 / A08 / A10 / A11 / A12、PR-006）の数値も `activity` から導出される（ランキング・CSV・X シェアは保持期間全体、利用時間の推移は直近 14 日、週次・月次はその週・その月、解除後の時間は最後に解除した日から今日まで）。種は `makeActivity()` で置き、対象のサイトを `makeSites()` にも入れて追跡中にする
- fixture の settings は必ず `makeAppSettings()`、追跡中のサイト（ブロック設定・時間制限・YouTube 機能）は `makeSites()` 経由で作る。フィールドが欠けると実装側のスキーマ検証に落ちて既定値へフォールバックし、「設定したのに効かない」という分かりにくい失敗になる。サイトのキーはサイトキー（`*.` / `www.` なし）で書く
- storage の読み書きヘルパー（`tests/e2e/helpers/storage.ts`）はキーごとに実装の型で引数を受ける。保存形と違うキー名は `pnpm type-check` で止まるので、`as any` を挟んで回避しない
- 滞在時間を書くのは heartbeat（`tracker-heartbeat`。表示されているページ）の 1 本だけで、書く先は `activity` の今日の行の `seconds`。記録されるのは追跡中のサイトだけなので、`makeSites()` で前提データを用意する。前面のウィンドウかどうかには依存しない。ブロック回数・解除回数も同じ行（`blocks` / `unblocks`）に入り、今日の行の 1 サイト分は `getTodayActivityViaSW`（`tests/e2e/helpers/sw.ts`）で読む
- 分析データのリセット（AN-007 / OPT-A09）は background が `activity` を今日の分も含めて消す（キーごと無くなる）。リセット前に種が入っていることを確かめてから、消えるのを poll で待つ
- 「存在しない要素が無いこと」で合否を決めない。実装に一度も無かったセレクタや削除済みのセレクタの不在は、何を壊しても成立する。実装にある要素・値を正面から確かめる
- テストが自分で `chrome.storage.local` に書いて同じキーを読み返すだけのテストは置かない。`src/` を全削除しても通る（確かめているのは Chrome のストレージであって本製品ではない）
- 失敗を合格に変換しない。`isVisible()` を catch で包んで真偽値にすると、strict mode 違反（一致が複数）もタイムアウトも「表示されていない」として飲み込む。一致が 1 件になる範囲まで絞ってから判定する
- 広すぎる一致で判定しない。`text=/5/i` のようなページ全体の数字一致や `toContainText('0')`（10 / 100 でも通る）は、壊れても落ちない。文言は `UI_TEXT`（`tests/e2e/helpers/constants.ts`）から、要素は data-testid から指す
- 範囲を絞るのに `xpath=../..` のような「何階層上」を使わない。構造が変わると範囲が広がり、ページのどこかに文字列があるだけで通る。指す先が無くなれば落ちる書き方（見出しからドキュメント順にたどる等）にする
- 固定 `setTimeout` で待たない。ブロックルールは `waitForBlockRules` / `waitForNoBlockRules`、それ以外の反映は `expect.poll`。⚠ poll の対象は**狙った経路だけが書く値**にする（別の経路も書く値だと、検証したい処理が走る前に条件が満たされる）
- `locator.count()` は自動リトライしない。件数は `toHaveCount`、下限だけを見るなら `expect.poll(() => locator.count())` で待つ

### 開発支援（投げ銭）

| ID      | シナリオ                                                 | 優先度 |
| ------- | -------------------------------------------------------- | ------ |
| SUP-001 | ヘルプタブに支援セクションが表示される                   | P2     |
| SUP-002 | 支援ボタンで Buy Me a Coffee のページが新しいタブで開く  | P2     |
| SUP-003 | 分析タブのレポート下に支援誘導が表示され、閉じると消える | P2     |
| SUP-004 | 一度閉じると 30 日は再表示されない                       | P2     |
| SUP-005 | 新規タブ・ブロック画面には支援導線を置かない             | P1     |

### 機能間相互作用

| ID      | シナリオ                                                  | 優先度 |
| ------- | --------------------------------------------------------- | ------ |
| INT-001 | Pause + Time Limit 同時有効時、Pause が優先される         | P1     |
| INT-002 | Pause + Schedule 同時有効時、Pause が優先される           | P1     |
| INT-003 | Schedule 有効中でも Time Limit 未超過ならブロックされない | P1     |
| INT-004 | Pause + Time Limit + Schedule 同時有効時の優先順位        | P1     |
| INT-005 | Analytics Opt-Out でも解除後の滞在時間は記録される        | P1     |
| INT-006 | パスワード保護 + Pause トグル の認証フロー                | P1     |
| INT-007 | パスワード保護 + ブロック解除 の認証フロー                | P1     |

## 3. テストシナリオ詳細

### POP-001: ポップアップが正常に表示される

**前提条件**

- 拡張機能がインストールされている

**手順**

1. ツールバーのVisionFocusアイコンをクリック

**期待結果**

- ポップアップウィンドウが表示される
- Header（ロゴ・設定・Pauseトグル）が表示される
- QuickBlockButton が表示される
- GoalCard が表示される
- 今日のサマリー（ブロック回数・トップブロックサイト）が表示される

---

### NEW-001: 新規タブを開くとダッシュボードが表示される

**前提条件**

- プリセットが1件以上設定されている

**手順**

1. 新規タブを開く

**期待結果**

- 背景画像が表示される
- 目標テキストが中央に表示される
- ミニ統計カードが表示される
- 設定アイコンが右下に表示される

---

### NEW-008: プリセット未設定時も通常のダッシュボードとCTAが表示

**前提条件**

- プリセットが1件も設定されていない

**手順**

1. 新規タブを開く

**期待結果**

- プリセット設定済みのときと同じダッシュボードが表示される（既定の背景・文字色が使われる）
- 目標テキスト（未設定なら設定を促す案内）が表示される
- ミニ統計カードが表示される
- ブロックサイトリストが表示される
- 「初めてのスタイルを作成」CTA が表示される（プリセットが1件でもあれば表示されない）

---

### BLOCK-001: ブロックリストに追加したサイトが newtab.html にリダイレクト

**前提条件**

- ブロックリストが空

**手順**

1. オプション画面を開く
2. ブロックリストタブを選択
3. 入力フィールドに「example.com」を入力
4. Enter キーまたは追加ボタンをクリック
5. 新しいタブで「https://example.com」にアクセス

**期待結果**

- newtab.html にリダイレクトされる
- ブロック情報（domain, count）が表示される
- 「サイトがブロックされました」メッセージが表示される

---

### OPT-B02: ドメインを入力してブロックリストに追加できる

**前提条件**

- オプション画面が開いている
- ブロックリストタブが選択されている

**手順**

1. 入力フィールドに「reddit.com」を入力
2. Enter キーまたは追加ボタンをクリック

**期待結果**

- ブロックリストに「reddit.com」が追加される
- リストに新しい項目が表示される
- ドメインリストアイテムにブロックスタイル（Always Blocked）が表示される

---

### TL-003: Time Limit 超過時に newtab.html へリダイレクトされる

**前提条件**

- ブロックリストに「youtube.com」が登録されている
- Daily Time Limit が 10秒 に設定されている
- 10秒以上 YouTube に滞在している

**手順**

1. YouTube にアクセスする
2. 10秒以上待つ

**期待結果**

- newtab.html へリダイレクトされる
- URL パラメータに `?reason=time_limit_exceeded` が含まれる
- 「時間制限に達しました」メッセージが表示される

---

### YT-001: YouTube Shorts を非表示にできる

**前提条件**

- オプション画面が開いている
- ブロックリストタブが選択されている

**手順**

1. YouTube セクションの「Hide Shorts」をオンにする
2. YouTube にアクセスする

**期待結果**

- YouTube Shorts の棚が表示されない
- Shorts タブが表示されない
- Shorts 動画（ytd-video-renderer[is-shorts]）が表示されない

---

### OPT-ST03: プリセットを選択・適用できる

**前提条件**

- スタイルタブが開いている
- プリセットが2件以上存在する

**手順**

1. プリセットセレクターでプリセットを選択
2. 「このスタイルを適用」ボタンをクリック

**期待結果**

- activePresetId が更新される
- 新規タブを開くと選択したプリセットが適用される
- 背景・目標テキスト・色が変更される

---

### OPT-S10: 設定した時間帯に該当プリセットが自動適用される

**前提条件**

- プリセット「仕事モード」が作成されている
- スケジュール「平日集中」が作成されている
  - 時間帯: 09:00-18:00
  - 曜日: 月-金
  - プリセット: 仕事モード

**手順**

1. 月曜日 09:00-18:00 の時間帯に新規タブを開く

**期待結果**

- 「仕事モード」プリセットが適用される
- スケジュール時間外は通常のプリセット（activePresetId）が適用される

---

### OPT-S13: 重複スケジュール（同時刻・同曜日）が設定された場合にエラー表示

**前提条件**

- スケジュールが1件以上存在する
  - 月曜日 09:00-12:00、プリセット「仕事モード」

**手順**

1. スケジュールタブを開く
2. 「スケジュールを追加」をクリック
3. 月曜日 10:00-13:00、プリセット「休憩モード」を設定
4. 保存ボタンをクリック

**期待結果**

- エラーメッセージが表示される（「既存のスケジュールと重複しています。」）
- モーダルは閉じたままにならず、入力を直して保存し直せる
- 重複するスケジュールが保存されない

---

### OPT-S14: プリセット削除時、該当スケジュールのスタイル連携が解除される

**前提条件**

- プリセット「仕事モード」が存在する
- スケジュール「平日集中」がプリセット「仕事モード」に紐づいていて、有効になっている

**手順**

1. スタイルタブを開く
2. プリセット「仕事モード」を削除
3. 確認モーダルで削除を確定する
4. スケジュールタブを開く

**期待結果**

- 手順 2 で確認モーダルが開き、参照しているスケジュールの件数が示される
  （参照しているスケジュールが 0 件のプリセットでは確認モーダルは開かず、そのまま削除される）
- 確認モーダルをキャンセルした場合、プリセットもスケジュールも変わらない
- スケジュール「平日集中」は削除されず、有効・無効の状態も変わらない
- スケジュール「平日集中」のスタイル表示が消える（「不明なスタイル」は表示されない）

---

### TL-008: Pause 有効中は Time Limit 超過してもブロックされない

**前提条件**

- ブロックリストに「youtube.com」が登録されている
- Daily Time Limit が 10秒 に設定されている
- Pause が有効化されている

**手順**

1. YouTube にアクセスする
2. 10秒以上待つ

**期待結果**

- Time Limit 超過してもリダイレクトされない
- Pause が優先される

---

### TL-009: Daily の使用実績は日付が変わるとリセットされる

**前提条件**

- ブロックリストに「youtube.com」が登録されている
- Daily Time Limit が 60秒 に設定されている
- 23:59:55 時点で 55秒 使用している

**手順**

1. 23:59:55 に YouTube にアクセスする
2. 10秒間待つ（00:00:05 に到達）

**期待結果**

- 00:00:00（端末のローカル時刻）を過ぎると使用量は 0 から数え直しになる（使用量は今日の行を読むだけで、リセット処理は無い）
- リダイレクトされない

---

### TL-011: 複数サイトで異なる Time Limit が同時に動作する

**前提条件**

- ブロックリストに「youtube.com」（Daily 60秒）が登録されている
- ブロックリストに「twitter.com」（Daily 120秒）が登録されている

**手順**

1. YouTube にアクセスし、30秒滞在
2. Twitter にアクセスし、60秒滞在
3. YouTube にアクセスし、40秒滞在（合計 70秒）

**期待結果**

- YouTube のみリダイレクトされる（Daily 60秒 超過）
- Twitter はリダイレクトされない（Daily 120秒 未満）
- 使用量はサイト単位で数える（`www.` 付き・`m.` 付きのホストでの滞在も同じサイトの使用量に入る）

---

### YT-006: アクセスブロック無効時は Time Limit 超過でも隠さない

**前提条件**

- YouTube 機能（Shorts 非表示）だけが有効で、アクセスブロック（youtube.com のブロック設定）は無い
- youtube.com の今日の滞在秒数が長く記録されている

**手順**

1. YouTube にアクセスする

**期待結果**

- リダイレクトされない（アクセスブロックが有効なときだけ newtab へ飛ばす）
- 注入される CSS は設定で有効にした非表示のルールだけで、上限超過を理由に足されるルールは無い
- 滞在は `youtube.com` の今日の行に記録されるが、時間制限の判定と通知には使わない（ブロック設定があるときだけ時間制限を使う）

---

### YT-009: Hide Shorts + Time Limit 同時設定時に両方が機能する

**前提条件**

- YouTube セクションで「blockAccess」がオン
- YouTube セクションで「Hide Shorts」がオン
- YouTube Time Limit が 10秒 に設定されている

**手順**

1. YouTube にアクセスする
2. Shorts が非表示になっていることを確認
3. 10秒以上待つ

**期待結果**

- Shorts が非表示になる
- 10秒後にリダイレクトされる
- 「blockAccess」がオンでも、上限に達するまでは非表示の設定（Shorts / おすすめ / コメント / ホームフィード）が効く（#422）
- 「blockAccess」がオフのときは Time Limit を超過してもリダイレクトされない（#407）

---

### YT-010: blockAccess と Time Limit の併用（超過後にブロック）

**前提条件**

- YouTube セクションで「blockAccess」がオン
- YouTube Time Limit が 60秒 に設定されている

**手順**

1. Time Limit 未超過の状態で YouTube にアクセスする
2. Time Limit を超過させてから YouTube にアクセスする

**期待結果**

- 未超過のうちは YouTube を開ける
- 超過後はリダイレクトされる（reason: time_limit_exceeded）
- blockAccess と Time Limit を併用した場合はブロックリストと同じ意味論になる

---

### AN-010: Opt-Out でも解除したサイトの追跡と解除の記録は残る

**前提条件**

- Analytics Opt-In が拒否（`analyticsOptIn.enabled` が false）されている
- ブロックリストに「example.com」が登録されている（追跡中のサイトの有効なブロック設定）

**手順**

1. オプション画面のブロックリストタブを開く
2. 「example.com」を削除（長押しで確定）する

**期待結果**

- 「example.com」は追跡中のサイトに残り、ブロック設定だけが外れる（`sites['example.com'].block` が null）
- 事実の表（`activity`）の今日の行に「example.com」の解除が 1 回記録される

⚠ `analyticsOptIn` が止めるのは **GA4 への送信だけ**で、手元の集計（ブロック回数・使用時間・解除の記録）は
拒否しても続く（machina-gg/vision-focus#431 の判断）。AN-006 / INT-005 も同じ主題。

---

### INT-001: Pause + Time Limit 同時有効時、Pause が優先される

**前提条件**

- ブロックリストに「youtube.com」が登録されている
- Daily Time Limit が 10秒 に設定されている
- Pause が有効化されている

**手順**

1. YouTube にアクセスする
2. 10秒以上待つ

**期待結果**

- リダイレクトされない
- Pause が Time Limit より優先される

---

### INT-002: Pause + Schedule 同時有効時、Pause が優先される

**前提条件**

- スケジュール「平日集中」が有効（月曜日 09:00-18:00）
- Pause が有効化されている

**手順**

1. 月曜日 09:00-18:00 の時間帯に新規タブを開く

**期待結果**

- スケジュールのプリセットが適用されない
- Pause が Schedule より優先される

---

### INT-003: Time Limit + Schedule 同時有効時の動作確認

**前提条件**

- スケジュール「平日集中」が有効（月曜日 09:00-18:00）
- ブロックリストに「youtube.com」が登録されている
- Daily Time Limit が 10秒 に設定されている

**手順**

1. 月曜日 09:00-18:00 の時間帯に新規タブを開く
2. プリセット「仕事モード」が適用されていることを確認
3. YouTube にアクセスし、10秒以上待つ

**期待結果**

- スケジュールのプリセットが適用される
- Time Limit 超過後にリダイレクトされる
- 両方の機能が独立して動作する
- スケジュール外では Time Limit を超過していてもリダイレクトされない（スケジュールが 1 件以上あれば、時間制限の無い項目も含めてすべての項目はスケジュール内だけブロックする）

---

### INT-004: Pause + Time Limit + Schedule 同時有効時の優先順位

**前提条件**

- スケジュール「平日集中」が有効（月曜日 09:00-18:00）
- ブロックリストに「youtube.com」が登録されている
- Daily Time Limit が 10秒 に設定されている
- Pause が有効化されている

**手順**

1. 月曜日 09:00-18:00 の時間帯に新規タブを開く
2. YouTube にアクセスし、10秒以上待つ

**期待結果**

- リダイレクトされない
- スケジュールのプリセットが適用されない
- 優先順位: Pause > Time Limit / Schedule

---

### INT-005: Analytics Opt-Out でも解除後の滞在時間は記録される

**前提条件**

- Analytics Opt-In が拒否（`analyticsOptIn.enabled` が false）されている
- 「example.com」がブロック設定なしで追跡中のサイトに載っている

**手順**

1. 「example.com」を開いたままにする

**期待結果**

- 事実の表（`activity`）の今日の行の「example.com」の滞在秒数が増える（解除後の時間はここから導出される）

⚠ `analyticsOptIn` が止めるのは GA4 への送信だけで、手元の集計は拒否しても続く
（machina-gg/vision-focus#431 の判断。AN-006 / AN-010 も同じ主題）。

---

### INT-006: パスワード保護 + Pause トグル の認証フロー

**前提条件**

- パスワード「test1234」が設定されている
- Pause が無効化されている

**手順**

1. ポップアップを開く
2. Pause トグルをクリック
3. パスワード入力モーダルが表示される
4. 「test1234」を入力して確定

**期待結果**

- Pause が有効化される
- ブロック機能が一時停止される

---

### INT-007: パスワード保護 + ブロック解除 の認証フロー

**前提条件**

- パスワード「test1234」が設定されている
- ブロックリストに「example.com」が登録されている

**手順**

1. オプション画面を開く
2. ブロックリストタブを選択
3. 「example.com」の削除ボタン（ゴミ箱アイコン）をクリック
4. パスワード入力モーダルが表示される
5. 「test1234」を入力して確定

**期待結果**

- パスワード保護が有効なときは、長押しの Unblock 確認モーダルではなくパスワードモーダルが出る
- 認証が通ると「example.com」が削除され、保存済みのブロックリストからも消える

---

## 4. 優先度の定義

| 優先度 | 説明                                                 | MVP対象 |
| ------ | ---------------------------------------------------- | ------- |
| P0     | 必須：クリティカルパス、コア機能、リリースブロッカー | ✅      |
| P1     | 重要：主要機能、エラーハンドリング、UX               | ✅      |
| P2     | 推奨：エッジケース、付加機能、細かいUX               | -       |

## 5. テスト実装の優先順位

### Phase 1: コア機能（P0 - MVP必須）

**ブロック機能の基本動作**

1. BLOCK-001: サイトブロック基本動作（declarativeNetRequest）
2. BLOCK-008: declarativeNetRequest でリダイレクト実行
3. NEW-001: 新規タブでダッシュボード表示
4. NEW-008: プリセット未設定時のダッシュボード表示とCTA
5. OPT-001, OPT-002: オプション画面の基本表示・タブ切り替え

**ブロックリスト管理** 6. OPT-B01, OPT-B02: ブロックリストタブ表示・ドメイン追加 7. OPT-B04: ブロックリストからドメイン削除

**ポップアップ基本機能** 8. POP-001: ポップアップ表示 9. POP-003: 目標カード表示 10. POP-004: 今日のサマリー表示

**スタイル設定（基本）** 11. OPT-ST01, OPT-ST02: スタイルタブ表示・プリセット一覧 12. OPT-ST03: プリセット選択・適用 13. OPT-ST06: 目標テキスト入力

**Pause 機能** 14. BLOCK-004, BLOCK-005: Pause トグルでブロック一時停止・解除 15. POP-008: Pause トグル動作

**スケジュール自動適用** 16. OPT-S10: スケジュールによるプリセット自動適用

### Phase 2: 主要機能（P1）

**Time Limit 機能**

1. TL-003: Time Limit 超過時のリダイレクト
2. TL-009: Time Limit の使用実績リセット（日付変更）
3. TL-006: 残り時間バッジ表示（ポップアップ）
4. TL-008: Pause 有効中の Time Limit 挙動
5. TL-009: リセット境界値テスト
6. TL-011: 複数サイトでの Time Limit 動作
7. OPT-B06: Time Limit 設定（オプション）

**YouTube ブロック機能** 6. YT-001: YouTube Shorts 非表示 7. YT-002: YouTube Recommendations 非表示 8. YT-004: YouTube 完全ブロック（blockAccess）10. YT-006: アクセスブロック無効時の Time Limit 不適用 12. YT-009, YT-010: YouTube 複合モード・時間制限との併用 13. OPT-B10, OPT-B11, OPT-B12: YouTube セクション表示・設定・Time Limit

**ブロックリスト拡張機能** 14. BLOCK-002: ワイルドカードブロック 15. BLOCK-006, BLOCK-007: ブロックアイテムの有効/無効切り替え 16. OPT-B03: ワイルドカード入力 17. OPT-B05: ブロックアイテム有効/無効切り替え（UI）18. OPT-B07: パスワード保護時の認証 19. OPT-B08: Unblock 確認モーダル

**スタイル設定（詳細）** 20. OPT-ST04, OPT-ST05: プリセット作成・削除 21. OPT-ST07: テキスト色選択 22. OPT-ST08, OPT-ST09: 背景タイプ・画像選択 23. OPT-ST11: フォント設定

**スケジュール管理** 24. OPT-S01, OPT-S02, OPT-S03: スケジュールタブ・週間カレンダー・スケジュール追加 25. OPT-S04, OPT-S05, OPT-S06: 時間帯・曜日・プリセット連携設定 26. OPT-S07, OPT-S08, OPT-S09, OPT-S11: スケジュール編集・削除・有効/無効切り替え・視覚フィードバック 27. OPT-S13, OPT-S14: スケジュール競合検出・プリセット削除時の連動処理

**Analytics 機能** 28. AN-001: サイト別ブロック回数記録 29. AN-002: ブロックリストから外したサイトの追跡継続 30. AN-003: 解除サイト滞在時間記録（Heartbeat）31. AN-005: Analytics Opt-In モーダル 32. AN-010: Opt-Out でも解除の記録を継続 33. OPT-A01, OPT-A02, OPT-A03: 分析タブ・ランキング・Unblock History 表示 34. OPT-A04, OPT-A05: 解除サイト再ブロック・トラッキング停止 35. OPT-B09: ブロック回数表示（各ドメイン）

**パスワード保護** 36. OPT-SET02, OPT-SET03, OPT-SET04, OPT-SET05: パスワード設定・変更・削除 37. POP-009: パスワード認証（Pause）

**多言語対応** 38. I18N-001, I18N-002: ブラウザ言語の自動検出 39. I18N-003: ヘッダーの操作は実装どおりのものだけ 40. I18N-004: 全画面統一

**機能間相互作用** 41. INT-001, INT-002, INT-003, INT-004: Pause/Time Limit/Schedule の優先順位 42. INT-005: Analytics Opt-Out でも滞在時間を記録 43. INT-006, INT-007: パスワード保護の認証フロー

**その他主要機能** 44. POP-010, POP-011: QuickBlock 機能 45. NEW-006, NEW-007: 目標テキスト編集・保存

### Phase 3: 追加機能（P2）

**追加機能**

1. PR-002, PR-003, PR-004, PR-005, PR-006, PR-007: 付加機能全般
2. OPT-ST10, OPT-ST12, OPT-ST13, OPT-ST14, OPT-ST15: スタイル機能
3. NEW-011: 壁紙ダウンロードボタン

**Analytics 詳細機能** 8. AN-004: 追跡中サイトのサブドメインの滞在時間 9. AN-006, AN-007: Opt-Out 時の集計継続・データリセット 10. OPT-A06, OPT-A07, OPT-A08, OPT-A09, OPT-A10, OPT-A11: Analytics 詳細機能

**その他詳細機能** 12. TL-007: Time Limit 使用状況表示 13. YT-003, YT-007: YouTube Comments・設定即時反映 14. BLOCK-009, BLOCK-010: ブロック履歴記録 15. NEW-009, NEW-010, NEW-012, NEW-013: ダッシュボード詳細表示 16. POP-007, POP-012, POP-013, POP-014: ポップアップ詳細機能 17. OPT-003, OPT-004: URL ハッシュ・Opt-In モーダル 18. OPT-SET07: 通知設定 19. OPT-H01, OPT-H02, OPT-H03: ヘルプタブ全般 20. OPT-SET01, OPT-SET06, OPT-SET08, OPT-SET09, OPT-SET10, OPT-SET11: 設定タブ全般

## 6. テストカバレッジ目標

| 優先度 | 目標カバレッジ |
| ------ | -------------- |
| P0     | 100%           |
| P1     | 80%            |
| P2     | 50%            |
